...
//Prototypes
int search(struct CircularQueue c,int pt);
void enqueue(struct CircularQueue *pw,struct Passenger p);
int dequeue(struct CircularQueue *pw);
void print(struct CircularQueue w,struct CircularQueueQueue c);
int isFull(struct CircularQueue w);
int isEmpty(struct CircularQueue w);

//Main function
void main(){
	//initialise
	struct Passenger p;
  	struct CircularQueue w;
  	struct CircularQueueQueue c;
  	int PNRLast = 0;
  	w.front = -1;
  	w.rear = -1;
  	c.front = -1;
...
void enqueueConfirmed(struct CircularQueueQueue *pc,struct Passenger p){
	//Boundary Conditions
	if(pc->rear == 99){
...
int search(struct CircularQueueQueue c,int pt){
	int rt = -1;
